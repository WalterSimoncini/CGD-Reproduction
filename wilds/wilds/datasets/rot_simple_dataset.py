...
def make_environment(num, frac, train=True):
    # The original code sampled groups using frac proportions,
    # resulting in a different dataset for each seed. In this
    # version we always create num * [frac_1, frac_2, frac_3]
    # items for each group, shuffle them and calculate
    # the labels as described in the CGD paper.
    ...
    np.random.shuffle(np_groups)
    gs = torch.from_numpy(np_groups)

    angle = 30 * np.pi / 180

    # The original code sampled 1k dimensions, but only
    # the first two dimensions had nonzero values. Here
    # we sample only two features as described in the
    # CGD paper.
    X = torch.randn([num, 2])
